...
export const ExplorePlaces = () => {
    ...
    useEffect(() => { document.title = "HomeAway: Explore stays"; }, []);

    const tempStay = {
        // "_id": "3a-61a8dfacc9866ab3dd55f213",
        "_id": "3a",
        ...
        *Hand sanitizer napkins supplied in all rooms.`,
        ...
        "loc": {
            ...
            "area": "Lev Hair",
            "zip": "",
            "pos": { "lat": 32.06196364406076, "lng": 34.77239418059386 },
    ...
    const stays = [];
    for (let i = 0; i < 10; i++) stays.push(JSON.parse(JSON.stringify(tempStay)));
        ...
        <section className="explore main-content content-wrapper">
            ...
            <section className="explore-content flex">
                <section ref={listRef} className="explore-listings active">
                    {stays.map((stay, idx) =>
                        <StayPreview key={`${stay._id}${idx + 1}`} stay={stay} />)}
